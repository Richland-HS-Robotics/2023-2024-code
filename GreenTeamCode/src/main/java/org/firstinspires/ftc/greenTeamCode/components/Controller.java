...
import org.firstinspires.ftc.greenTeamCode.components.Arm;
import org.firstinspires.ftc.robotcore.external.Telemetry;
import org.firstinspires.ftc.greenTeamCode.util.Pair;
...
//        );
    ...
    private Arm.Position lastPosition = Arm.Position.FOLDED;

    @OneDriverControl(key = {GamepadKey.DPAD_DOWN,GamepadKey.DPAD_LEFT,GamepadKey.DPAD_UP}, description = "Move the arm up, down, or fold")
    public Arm.Position armPosition(){
        if(currentGamepad1.dpad_down){
            lastPosition = Arm.Position.DOWN;
            return Arm.Position.DOWN;
        }
        if(currentGamepad1.dpad_left){
            lastPosition = Arm.Position.UP;
            return Arm.Position.UP;
        }
        if(currentGamepad1.dpad_up){
            lastPosition = Arm.Position.FOLDED;
            return Arm.Position.FOLDED;
        }
        return lastPosition;
    }
